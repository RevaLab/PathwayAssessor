...
def geometric_average_with_q(iterable, q):
    try:
        clean_iterable = [el for el in iterable if ~np.isnan(el) and el !=0]
        log_iterable = np.log(clean_iterable) * -1
        log_iterable.sort()
        if not len(clean_iterable):
            return np.nan
        if q == 1:
        ...
        if q == -1:
            ...
            if Qmin == 0:
                return 0
            N = len(log_iterable)
            denominator = 1 + sum([Qmin/Qi for Qi in log_iterable])
            return (N*Qmin) / denominator
    except ZeroDivisionError:
        return 0
...
def all(
    ...
    geometric_averages = []
    min_p_vals = []
    # geometric_averages_qneg1 = [None] * len(pathways)
    # geometric_averages_qpos1 = [None] * len(pathways)
    ...
    for i, pathway in enumerate(pathways):
        ...
        if geometric:
            ...
            # try:
                # geometric_averages_qneg1_series = p_values_df.apply(geometric_average_with_q, q=-1).loc[sample_order]
                # geometric_averages_qneg1_series.name = pathway
                # geometric_averages_qneg1[i] = geometric_averages_qneg1_series
                #
                # geometric_averages_qpos1_series = p_values_df.apply(geometric_average_with_q, q=1).loc[sample_order]
                # geometric_averages_qpos1_series.name = pathway
                # geometric_averages_qpos1[i] = geometric_averages_qpos1_series
            # except KeyError:
            #     print(pathway)
            #     print(p_values_df.apply(geometric_average_with_q, q=-1))
        if min_p_val:
...
#         break
    ...
    # geometric_averages_qneg1_df = pd.concat(geometric_averages_qneg1, axis=1).T
    # geometric_averages_qpos1_df = pd.concat(geometric_averages_qpos1, axis=1).T
    ...
    return {
               ...
               # 'geometric_qneg1': geometric_averages_qneg1_df,
               # 'geometric_qpos1': geometric_averages_qpos1_df,
    }
